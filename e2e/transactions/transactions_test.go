...
func (s *TransactionsTestSuite) TestJailSendQueuedTransaction() {
	s.StartTestBackend(params.RopstenNetworkID)
	defer s.StopTestBackend()
	...
	signal.SetDefaultNodeNotificationHandler(func(jsonEvent string) {
		...
		if envelope.Type == txqueue.EventTransactionQueued {
			...
			if requireMessageId {
				s.NotEmpty(messageId, "Message id is required, but not provided")
			} else {
				s.Empty(messageId, "Message id is not required, but provided")
			}
	...
	for _, test := range tests {
		jail.BaseJS(string(static.MustAsset(txSendFolder + test.file)))
		jail.Parse(testChatID, ``)
